...
'use strict';
    ...
    function load(urlSpec) {
        ...
        for (var f = 0; f < urls.length; ++f) {
            var url = urls[f];
            if (percy.EmbeddedFiles && percy.EmbeddedFiles.isEmbeddedURL(url))
    ...
    function loadSuccess(urlSpec, f, content) {
        ...
        urlSpec.content = percy.Util.arraysConcatAll(urlSpec.filesContent);       // Concat all files in order
    ...
    function scheduleLoadingIcon() {
        if (!Percy.room || Percy.room.isLoading) return;

        loadingTimer = window.setTimeout(function setLoadingOnDelay() {
            loadingTimer = null;
            loadingSet = true;
            Percy.room.setLoading(true);
